...
const SPOTS = [
  { id:'library', name:'새벽벌도서관', emoji:'📚', entry:'QM-V5-LIBRARY-ENTRY', exit:'QM-V5-LIBRARY-EXIT', prefix:'LIB', challenge:'READ' },
  { id:'meal', name:'금정회관', emoji:'🍚', entry:'QM-V5-MEAL-ENTRY', exit:'QM-V5-MEAL-EXIT', prefix:'MEAL', challenge:'RICE' },
  { id:'notice', name:'공지 확인 부스', emoji:'📣', entry:'QM-V5-NOTICE-ENTRY', exit:'QM-V5-NOTICE-EXIT', prefix:'NOTI', challenge:'INFO' },
  { id:'hidden', name:'캠퍼스 히든스팟', emoji:'🗺️', entry:'QM-V5-HIDDEN-ENTRY', exit:'QM-V5-HIDDEN-EXIT', prefix:'HIDE', challenge:'FIND' },
  { id:'team', name:'팀 챌린지 존', emoji:'🤝', entry:'QM-V5-TEAM-ENTRY', exit:'QM-V5-TEAM-EXIT', prefix:'TEAM', challenge:'CREW' }
];
const SECRET = process.env.QUESTMATE_CHECKIN_SECRET || 'pnu-questmate-v5-demo-secret';
const DAILY_LIMIT = 2;
...
function checkLimits(spot, logs) {
  ...
  if (logs && logs.length) {
    const last = Math.max(...logs.map(x => Number(new Date(x.issuedAt))).filter(Boolean));
    if (last && Date.now() - last < COOLDOWN_MS) throw new Error('연속 인증 방지를 위해 마지막 인증 후 45분이 지나야 합니다.');
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'start') {
      ...
      return json(200, { ok:true, message:'입장 QR·현장코드·문구 검사를 통과했습니다. 이제 최소 체류시간 후 퇴장 QR을 인증하세요.', visit: { ...payload, id: tokenFor(payload) } });
    }
    if (action === 'complete') {
